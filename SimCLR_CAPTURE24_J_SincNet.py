...
# ## CAPTURE24 Dataset
# 
# In this section, the CAPTURE24 dataset will be downloaded and parsed. The results will then be saved in a python pickle file.
...
# %%
with zipfile.ZipFile(working_directory + 'capture24.zip', 'r') as zip_ref:
...
user_datasets = raw_data_processing.process_capture24_accelerometer_files(accelerometer_data_folder_path)

# %%
with open(working_directory + 'capture24_user_split.pkl', 'wb') as f:
    pickle.dump({
...
with open(working_directory + 'capture24_user_split.pkl', 'rb') as f:
        data = pickle.load(f)
...
dataset_name = 'capture24.pkl'
dataset_name_user_split = 'capture24_user_split.pkl'

label_list = ['null', 'sleep', 'sedentary', 'light', 'moderate-vigorous']
...
model_save_name = f"capture24_acc"

sampling_rate = 100.0 

# a fixed user-split
test_users_fixed = list(range(122, 151)) # first 121 users for training, rest for testing
...
transform_funcs = [
    ...
    transformations.noise_transform_vectorized # Use rotation trasnformation
]
...
base_model = simclr_models.create_sincnet_base_model(input_shape=input_shape, model_name="SincNet", num_sinc_filters=4, sinc_kernel_size=201, sample_rate=sampling_rate, depthwise=True)
